# ArtMind Backend - Demo script cho 11 nhÃ³m chá»©c nÄƒng
# CÃ¡ch dÃ¹ng:
...
function Write-Console {
    ...
    $previousColor = $hostUi.ForegroundColor
    try {
        $hostUi.ForegroundColor = $ForegroundColor
        Write-Information -MessageData $Message -InformationAction Continue
    } finally {
        $hostUi.ForegroundColor = $previousColor
    }
}
...
function Write-Section($num, $title) {
    ...
    Write-Console " [$num] $title" -ForegroundColor Cyan
    Write-Console "========================================" -ForegroundColor Cyan
}
...
function Test-Api {
    ...
    try {
        $result = & $Action
        Write-Console "  PASS  $Name" -ForegroundColor Green
        if ($result) { Write-Console "        -> $result" -ForegroundColor DarkGray }
        $script:Pass++
    } catch {
        $msg = $_.ErrorDetails.Message
        if (-not $msg) { $msg = $_.Exception.Message }
        Write-Console "  FAIL  $Name" -ForegroundColor Red
        Write-Console "        -> $msg" -ForegroundColor DarkRed
        $script:Fail++
    }
}

Write-Console "ArtMind Backend Demo - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Yellow
...
Test-Api "GET /paintings/p3 (chi tiet)" {
    $r = Invoke-RestMethod -Uri "$Base/paintings/p3"
    return "$($r.data.title) by $($r.data.artist)"
}

Test-Api "GET /paintings/p3/similar" {
    $r = Invoke-RestMethod -Uri "$Base/paintings/p3/similar"
    return "$($r.data.Count) tranh tuong tu"
}

Test-Api "GET /paintings/p3/summary (AI)" {
    $r = Invoke-RestMethod -Uri "$Base/paintings/p3/summary"
    $short = $r.data.summary.Substring(0, [Math]::Min(60, $r.data.summary.Length))
    return "cached=$($r.data.cached): $short..."
}

Test-Api "POST /paintings/p3/view (tang luot xem)" {
    $r = Invoke-RestMethod -Method Post -Uri "$Base/paintings/p3/view" -Headers $Headers
...
Test-Api "POST /paintings/p3/auto-tag (admin AI tag)" {
    $r = Invoke-RestMethod -Method Post -Uri "$Base/paintings/p3/auto-tag" -Headers $Headers
...
Test-Api "POST /users/me/favorites/p3" {
    $r = Invoke-RestMethod -Method Post -Uri "$Base/users/me/favorites/p3" -Headers $Headers
    return $r.message
}
...
Test-Api "POST /users/me/recent/p3" {
    $r = Invoke-RestMethod -Method Post -Uri "$Base/users/me/recent/p3" -Headers $Headers
    return $r.message
}
...
Test-Api "POST /analytics/track" {
    $body = @{ eventType = "click"; paintingId = "p3"; metadata = @{ source = "demo" } } | ConvertTo-Json
    $null = Invoke-RestMethod -Method Post -Uri "$Base/analytics/track" -ContentType "application/json" -Body $body -Headers $Headers
...
Test-Api "GET /paintings/p3/export/pdf" {
    $out = Join-Path $ExportDir "ArtMind_p3_demo.pdf"
    Invoke-WebRequest -Uri "$Base/paintings/p3/export/pdf" -OutFile $out
    $size = (Get-Item $out).Length
    return "saved $out ($size bytes)"
}

Test-Api "GET /paintings/p3/export/docx" {
    $out = Join-Path $ExportDir "ArtMind_p3_demo.docx"
    Invoke-WebRequest -Uri "$Base/paintings/p3/export/docx" -OutFile $out
    $size = (Get-Item $out).Length
    return "saved $out ($size bytes)"
}
...
Test-Api "POST /paintings (tao moi)" {
    $body = @{
        ...
        surface = "Canvas"
        price = 999
        image_url = "https://upload.wikimedia.org/wikipedia/commons/a/ae/Victory_in_Battle_of_Dien_Bien_Phu.jpg"
...
Write-Console " KET QUA: $Pass PASS | $Fail FAIL" -ForegroundColor $(if ($Fail -eq 0) { "Green" } else { "Yellow" })
Write-Console "========================================" -ForegroundColor Yellow
Write-Console ""
Write-Console "File export demo: $ExportDir" -ForegroundColor DarkGray
Write-Console "Chup man hinh output nay de nop bao cao / demo video." -ForegroundColor DarkGray
