...
export default function MainLayout({ children }) {
    const { searchQuery, setSearchQuery, executeSmartSearch, user, logout } = useArtStore();
        ...
        <div className="min-h-screen bg-[#0b0d19] text-slate-100 font-sans relative overflow-x-hidden selection:bg-purple-500 selection:text-white">
            ...
            <div className="relative z-20 bg-gradient-to-r from-purple-900/30 via-[#121629] to-purple-900/30 border-b border-purple-500/20 py-2 text-center shadow-lg">
                <p className="text-[11px] text-slate-300 font-medium">
                    {user ? (
                      <>Xin chào, <strong className="text-purple-300">{user.firstName || user.username}</strong>! Chúc bạn khám phá nghệ thuật vui vẻ ✨</>
                    ) : (
                      <>
                    ...
                    <div className="ml-2 pl-2 border-l border-slate-800 flex items-center gap-2">
                        {user ? (
                          <>
                            ...
                            <img
                                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.username || 'U')}&background=7c3aed&color=fff`}
                                alt="User"
